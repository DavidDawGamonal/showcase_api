 ...
 * Permite crear y eliminar playlists, añadir y quitar tracks de ellas, y
 * obtener las playlists de un usuario con sus tracks.
 *
...
public class PlaylistService {
	 ...
	 * @param likeRepository          Repositorio de likes — para contar likes de
	 *                                cada track
	 * @param comentarioRepository    Repositorio de comentarios — para contar
	 *                                comentarios de cada track
	 */
	public PlaylistService(PlaylistRepository playlistRepository, PlaylistTrackRepository playlistTrackRepository,
			TrackRepository trackRepository, UsuarioRepository usuarioRepository, LikeRepository likeRepository,
	...
	/**
	 * Crea una nueva playlist para un usuario. Comprueba que el usuario no tenga ya
	 * una playlist con el mismo nombre antes de persistir — la BD también tiene la
	 * restricción unique_nombre_usuario pero es mejor validarlo en el service para
	 * devolver un mensaje claro.
	 *
	 * @param idUsuario ID del usuario propietario
	 * @param nombre    Nombre de la nueva playlist
	 * @return true si se creó correctamente, false si ya existe una playlist con
	 *         ese nombre
	 */
	...
	/**
	 * Elimina una playlist por su id. Los tracks de la playlist se eliminan en
	 * cascada gracias a la FK de la BD.
	 *
	...
	/**
	 * Añade un track a una playlist. Comprueba que el track no esté ya en la
	 * playlist antes de añadirlo.
	 *
	 * @param idPlaylist ID de la playlist
	 * @param idTrack    ID del track a añadir
	 * @return true si se añadió correctamente, false si el track ya estaba en la
	 *         playlist
	 */
	...
	/**
	 * Devuelve todas las playlists de un usuario con sus tracks. Convierte cada
	 * Playlist a PlaylistData y cada track a TrackResumen para no exponer las
	 * entidades directamente al frontend.
	 *
	...
	public List<PlaylistData> getPlaylistsByUsuario(Integer idUsuario) {
		...
		for (Playlist playlist : playlists) {
			...
			resultado.add(new PlaylistData(playlist.getIdPlaylist(), playlist.getNombre(), playlist.getFecCreacion(),
					tracks));
		}
		return resultado;
	}
