...
/**
 * Servicio que gestiona la lógica de negocio relacionada con los tracks
 * musicales. Proporciona operaciones de subida de archivos, consulta y búsqueda
 * de tracks.
 *
 * <p>
 * Gestiona dos tipos de archivos en disco:
 * </p>
 * <ul>
 * <li>Instrumentales: archivos de audio almacenados en la carpeta
 * "instrumentales"</li>
...
public class TrackService {
	...
	/**
	 * Nombre del archivo de portada por defecto cuando el productor no sube
	 * ninguna.
	 */
	private static final String PORTADA_DEFAULT = "portada_default.jpg";

	/**
	 * Constructor que inyecta los repositorios necesarios e inicializa las rutas de
	 * almacenamiento.
	 *
	 ...
	 * @return true si el track se subió y guardó correctamente, false si hubo algún
	 *         error
	 */
		...
		try {
			...
			if (rutaArchivo != null && rutaPortada != null) {
				Track track = new Track(dataTrack.getTitulo(), dataTrack.getGenero(), dataTrack.getDuracion(),
						dataTrack.getDescripcion(), rutaPortada, rutaArchivo, usuario);
	...
	/**
	 * Guarda la imagen de portada del track en disco. Si el productor no sube
	 * portada, asigna la portada por defecto. Los espacios en el nombre del archivo
	 * se reemplazan por guiones para evitar problemas en rutas y URLs.
	 *
	...
	private String subirPortada(Usuario usuario, DataTrack dataTrack) {
		...
		try {
			...
			if (tienePortada) {
				// Los espacios en el nombre se sustituyen por guiones para evitar problemas en
				// rutas
	...
	/**
	 * Guarda el archivo de audio instrumental del track en disco. Crea la carpeta
	 * del usuario si no existe y almacena el archivo en ella.
	 *
	 * @param usuario   El usuario productor que sube el track
	 * @param dataTrack DTO con los datos del track incluyendo el archivo de audio
	 * @return La ruta relativa donde se guardó el instrumental, o null si hubo un
	 *         error
	 */
	...
	/**
	 * Obtiene el listado de tracks según los filtros proporcionados. Lógica de
	 * filtrado: - Si hay idUsuario y género: carga los top 5 del productor con ese
	 * género; si no llega a 5, completa con tracks del mismo género de otros
	 * productores. - Si hay idUsuario sin género: carga los top 5 del productor por
	 * likes. - Si no hay idUsuario: carga el top 5 global por likes. Cada
	 * TrackResumen incluye el número de likes y comentarios calculados.
	 *
	...
	/**
	 * Busca tracks cuyo título contenga el texto indicado (insensible a
	 * mayúsculas). Cada TrackResumen incluye el número de likes y comentarios
	 * calculados.
	 *
...
//	}
	...
	/**
	 * Sustituye la de aquí encima Obtiene un track por su id y lo devuelve como
	 * TrackResumen. Lanza una excepción HTTP 404 si el track no existe, en lugar de
	 * un NullPointerException como hacía la versión anterior (comentada arriba).
	 *
	...
	/**
	 * Obtiene los 10 tracks subidos más recientemente, ordenados por fecha de
	 * subida descendente. Cada TrackResumen incluye el número de likes y
	 * comentarios calculados.
	 *
	 * PENDIENTE — filtrar por última semana: requiere nueva query en
	 * TrackRepository findByFecSubidaAfter(@Param("desde") LocalDateTime desde,
	 * Pageable pageable) para filtrar en BD antes del PageRequest. Las líneas
	 * comentadas son un primer paso pero insuficientes solas.
	 *
	...
	/**
	 * Devuelve el número total de reproducciones de todos los tracks de un
	...
	/**
	 * Devuelve el número total de tracks subidos por un productor. Se usa para
	 * mostrar las estadísticas globales del productor en su perfil.
	 *
	 * @param idUsuario ID del productor
	...
	/**
	 * Busca tracks aplicando filtros avanzados con filtrado en memoria. Se usa
	 * exclusivamente desde la sección "Explorar" de principal.html.
	 *
	 * <p>
	 * A diferencia de getListadoTracks(), este método no limita los resultados a 5
	 * y permite filtrar por cualquier combinación de criterios dinámicos. El
	 * filtrado en memoria es válido para el volumen de datos del proyecto y evita
	 * tener que crear una query específica por cada combinación posible de filtros.
	 * </p>
	 *
	 * <p>
	 * Orden de filtrado:
	 * </p>
	 * <ol>
	 * <li>Filtros sobre la entidad Track (género, idUsuario, nombreProductor,
	 * fechas)</li>
	 ...
	 * @return Lista de TrackResumen con los tracks que cumplen todos los filtros
	 *         aplicados
	 */
	public List<TrackResumen> buscarConFiltros(FiltroTrack filtro) {
		...
		if (filtro.getGenero() != null && !filtro.getGenero().isEmpty()) {
			tracks = tracks.stream().filter(t -> filtro.getGenero().equalsIgnoreCase(t.getGenero()))
					.collect(Collectors.toList());
		}

		// Filtro por idUsuario (productor)
		if (filtro.getIdUsuario() != null && filtro.getIdUsuario() > 0) {
			tracks = tracks.stream().filter(t -> t.getUsuario().getIdUsuario() == filtro.getIdUsuario())
					.collect(Collectors.toList());
		}
		...
		if (filtro.getNombreProductor() != null && !filtro.getNombreProductor().isEmpty()) {
			tracks = tracks.stream().filter(
					t -> t.getUsuario().getNombre().toLowerCase().contains(filtro.getNombreProductor().toLowerCase()))
					.collect(Collectors.toList());
		}

		// Filtro por fecha de subida desde (inclusive)
		if (filtro.getFechaDesde() != null) {
			tracks = tracks.stream().filter(t -> !t.getFecSubida().toLocalDate().isBefore(filtro.getFechaDesde()))
					.collect(Collectors.toList());
		}

		// Filtro por fecha de subida hasta (inclusive)
		if (filtro.getFechaHasta() != null) {
			tracks = tracks.stream().filter(t -> !t.getFecSubida().toLocalDate().isAfter(filtro.getFechaHasta()))
					.collect(Collectors.toList());
		}

		// Convertimos a TrackResumen para poder filtrar por likes y comentarios reales
		// Los likes y comentarios no están en la entidad Track — se calculan desde los
		// repositorios
		List<TrackResumen> resultado = tracks.stream()
				.map(t -> new TrackResumen(t, likeRepository.countByTrackIdTrack(t.getIdTrack()),
						comentarioRepository.countByTrackIdTrack(t.getIdTrack())))
				.collect(Collectors.toList());

		// Filtro por likes mínimos — se aplica después de convertir porque likes no
		// está en la entidad
		if (filtro.getMinLikes() != null) {
			resultado = resultado.stream().filter(t -> t.getLikes() >= filtro.getMinLikes())
					.collect(Collectors.toList());
		}

		// Filtro por comentarios mínimos — igual que likes, no está en la entidad
		if (filtro.getMinComentarios() != null) {
			resultado = resultado.stream().filter(t -> t.getComentarios() >= filtro.getMinComentarios())
					.collect(Collectors.toList());
		}
	...
	/**
	 * Devuelve los tracks a los que un usuario ha dado like, ordenados del más
	 * reciente al más antiguo. Se usa para mostrar la sección "Mis favoritos" en el
	 * perfil del usuario. Devuelve FavoritoData en lugar de TrackResumen para
	 * incluir la fecha del like y seguir el patrón DTO del proyecto — no exponer
	 * entidades directamente al frontend. Incluye el número real de likes de cada
	 * track consultando el repositorio por cada uno.
	 *
...
//                    : "portadas@portada_default.jpg";
			...
			String duracion = track.getDuracion().format(java.time.format.DateTimeFormatter.ofPattern("HH:mm:ss"));

			resultado.add(new FavoritoData(track.getIdTrack(), track.getTitulo(), track.getUsuario().getNombre(),
					track.getPortada(), duracion, totalLikes, like.getFecLike()));
		}
		return resultado;
	}
}
