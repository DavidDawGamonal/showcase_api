 ...
 * Registra cada descarga en la BD, cuenta las descargas por usuario y por
 * track, y devuelve el historial de descargas de un usuario. Solo los usuarios
 * registrados pueden descargar tracks.
 *
...
public class DescargaService {
	...
	/**
	 * Registra una descarga en la BD y devuelve el archivo del track. Se llama cada
	 * vez que un usuario descarga un track.
	 *
	 ...
	 * mostrar las estadísticas del usuario en su perfil.
	 *
	...
	/**
	 * Devuelve el número total de descargas de un track. Se usa para mostrar la
	 * popularidad de un track.
	 *
	 ...
	 * descendente. Se usa para mostrar la sección "Mis descargas" en el perfil del
	 * usuario. Devuelve DescargaData en lugar de la entidad Descarga para evitar
	 * problemas de serialización con las relaciones lazy de JPA y seguir el patrón
	 * DTO del proyecto. Incluye el número real de likes de cada track consultando
	 * el repositorio por cada uno.
	 *
	 * @param idUsuario ID del usuario
	...
	public List<DescargaData> getDescargasByUsuario(Integer idUsuario) {
		...
		for (Descarga descarga : descargas) {
			...
			String portada = track.getPortada() != null ? track.getPortada().replace(File.separatorChar, '@')
			...
			resultado.add(new DescargaData(track.getIdTrack(), track.getTitulo(), track.getUsuario().getNombre(),
					portada, duracion, totalLikes, descarga.getFecDescarga()));
		}
		return resultado;
	}
}